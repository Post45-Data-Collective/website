...
::: {.callout-tip appearance="default"}
## **NA vs. N/A vs. NaN**
For reference, computational tools like Pandas will automatically read values like `NA`, `N/A`, `NaN`, `null`, `NULL`, `None`, and empty cells as the same missing-value (`NaN`) by default. If you use `N/A` to mean *not applicable* and `NaN` to mean *missing*, that distinction may disappear when someone loads the CSV in pandas (unless they use `keep_default_na=False`).
It's important to remember that different programs treat blank cells differently. Excel may render an empty cell as visually identical to a cell containing the string `"NaN"` or a single space. Without an explicit marker, it’s hard to tell what a blank cell was meant to communicate.
:::
...
An **ISBN** (International Standard Book Number) is a unique 10- or 13-digit identifier assigned to a specific edition of a book. An **ISSN** (International Standard Serial Number) is the equivalent 8-digit identifier for a serial publication like a journal or magazine. 

Including an ISBN or ISSN for each publication makes this metadata more accessible. If you can, **we prefer to include at least one rather than none**. Our [BookReconciler](https://data.post45.org/our-tools.html){.external target="_blank"} tool can help fetch related ISBNs for the books in your dataset alongside other identifiers.

::: {.callout-tip appearance="default"}
## **A note on ISBNs and “Works”**
ISBNs identify **a specific edition** of a book—hardcover or paperback, a translation or a reissue will each have their own ISBN. There's currently no single identifier that indicates whether these ISBNs refer to the same "Work."

"Work" is the abstract concept that unites every edition, translation, and reprint in bibliographic terminology (specifically the [FRBR](https://www.loc.gov/cds/downloads/FRBR.PDF){.external target="_blank"} model).

For more on this issue, see Matt Miller, Dan Sinykin, and Melanie Walsh, [“BookReconciler📘💎: An Open-Source Tool for Metadata Enrichment and Work-Level Clustering”](https://arxiv.org/abs/2512.10165){.external target="_blank"} (*Joint Conference on Digital Libraries*, 2025).
::: 

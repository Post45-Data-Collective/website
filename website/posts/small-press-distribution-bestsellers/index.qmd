...
viewof vizTopN = Inputs.range([5, 30], { label: "Number of authors", step: 1, value: 15 })
```
...
// Count appearances per author (each row = one appearance on a monthly list),
// optionally filtered by genre, and keep the top N.
topAuthors = {
  const rows = raw_data.filter(d => d.author && (vizGenre === "All" || d.genre === vizGenre));
...
html`<h3>Top ${topAuthors.length} Authors${vizGenre === "All" ? "" : " in " + vizGenre} by Appearances</h3>`
```
...
Plot.plot({
  ...
  marginRight: 40,
  width: 900,
  height: topAuthors.length * 26 + 50,
  x: { label: "Appearances on bestseller lists", grid: true },
  y: { label: null },
  marks: [
    Plot.barX(topAuthors, {
      x: "count",
      y: "author",
      sort: { y: "x", reverse: true },
      fill: "#4a6da7"
    }),
    Plot.text(topAuthors, {
      x: "count",
      y: "author",
      text: d => d.count,
      dx: 12,

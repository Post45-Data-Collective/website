...
**Update (10/31/2025):** Please note that the authors of this dataset and data essay updated both documents in October 2025. After discovering that several translations were missing, specifically concerning Norwegian, the authors augmented their data with information scraped from the website Goodreads. The revised dataset now has 1,661 additional rows, and two additional columns: `author_gender` and `translator`. The first (February 2024) version of the dataset is still accessible [here](https://github.com/Post45-Data-Collective/data/blob/main/nordic_noir_translation_database/nordic_noir_translation_database-v1.csv).
...
//| echo: false
//| output: false
nnitd_data = fetchData("https://raw.githubusercontent.com/Post45-Data-Collective/data/refs/heads/main/nordic_noir_translation_database/nordic_noir_translation_database.csv")
```
...
generateTabulatorTableFromCSV(
  "#table-container-nnitd",
  "https://raw.githubusercontent.com/Post45-Data-Collective/data/refs/heads/main/nordic_noir_translation_database/nordic_noir_translation_database.csv",
  {
    displayedColumns: ["author", "author_gender", "original_title", "original_language",
                       "original_publication_year", "translated_title", "translated_language",
                       "translated_publication_year", "translator"],
    columnPopups: [
      "The author of the translated book",
      "Gender of the author. m = male; f = female; mm = two male authors; ff = two female authors; fm = one female + one male; unknown = unidentified",
      "Title of the book in its original source language",
      "Source language of the original book",
      "Year of first publication in the source language",
      "Main title of the translated book",
      "Target language of the translation",
      "Year the book was published in translation",
      "The translator of the book"
    ],
    columnWidths: { "author_gender": 130, "original_language": 150, "translated_language": 150 },
    rangeSliderColumns: ["original_publication_year", "translated_publication_year"],
    categoryColumns: ["author_gender", "original_language", "translated_language"],
    sortColumns: ["original_publication_year"],
    sortOrders: ["desc"],
    buttonContainerId: "#button-container-nnitd",
    rawButtonId: "#download-raw-nnitd",
    urlCopyButtonId: "#copy-url-nnitd",
  }
);
```

<div id="table-container-nnitd" style="height: 600px"></div>

Download Full Data (including hidden columns)

<div>
  <button id="download-raw-nnitd"><i class="fas fa-download"></i> Download Full Data</button>
  <button id="copy-url-nnitd"><i class="fas fa-copy"></i> Copy Full Data URL</button>
</div>

Download Table Data (filtered rows / visible columns)

<div id='button-container-nnitd'>
...
In 2025, we discovered that some of our original data sources had significant gaps; this was particularly true for translation events based on Norwegian originals. In response, we developed a new web scraping methodology designed to capture the missing titles by extracting data from the online social cataloguing site Goodreads. Specifically, we scraped the “editions” page of each original title in the NNTD. We then manually deleted duplicates and corrected errors across all variables in the scraped data. As a result, we have added a total of 1,661 titles, of which 797 (48 percent) relate to Norwegian originals.

Concurrently with this scraping exercise, we also added the variables “author gender” and “translator” for all entries. For author gender, we relied on the pronouns used either by the authors themselves or their publishers and agents. We did not identify any non-binary authors, but will include this category in a later revision if required. 
...
**author_gender** ⇒ The gender of the translated book’s author. _m_ = male; _f_ = female; _mm_ = two male authors; _ff_ = two female authors; _fm_ = one female and one male author; _unknown_ = unknown or unidentified author gender.
...
The NNTD comprises bibliographic records of – in theory – all translations of Nordic crime fiction for adults, written in any of the five main Nordic languages (Danish, Finnish, Icelandic, Norwegian and Swedish) and published in translation during the period 2000–2020. The dataset is delimited to include only _first edition_ translations published in _print book form_. We have included the “in theory” qualification because we acknowledge that quantitative investigations of bibliographic data involve statistical loss, and that this general problem is exacerbated in the translation context because compilers of national bibliographies tend not to prioritise outbound translations. Accordingly, we concur with previous scholarship on global translation patterns (Heilbron 1999; Lindqvist 2016) that exact numbers are impossible to produce, and that any dataset related to global translation flows – including the NNTD – should be understood as evidence-based indications of tendencies. The international sources available for studying translation, such as the _Index Translationum_ (_IT_), unfortunately suffer from severe methodological problems and have large gaps (Heilbron 2000; Svedjedal 2012). Because the NNTD combines bibliographic information at the national level, which is generally much more reliable than the _IT_, with Goodreads data added by numerous users and harmonized by the platform, we are confident that it is as complete as can be achieved. For further discussion, see [our research article](https://doi.org/10.1080/14781700.2024.2333737) based on this dataset (Berglund, Gulddal and King).

The dataset is available in a matrix format as one csv file, consisting of 10,547 rows (one row per translation event) and nine columns. The columns cover the key parameters: author(s); author gender; original title; original year of publication; source language; title in translation; target language; and year published in translation; and translator. The dataset thus covers 10,547 translation events of Nordic crime fiction during the period 2000-2020, distributed over 408 unique authors and author constellations (plus one anthology), and 1,585 unique original titles.. Further descriptive statistics concerning the composition of the dataset can be found in [the associated research article](https://doi.org/10.1080/14781700.2024.2333737). These parameters are with very few exceptions complete for all rows in the dataset. All information has been checked manually against national biographical databases. The only exception is the column “title” (in translation), where such checking was not always possible for practical reasons.

It would have been valuable to include other parameters in the dataset, including original publisher and publisher of the translation. However, the nature of our sources made it impossible to collect this data in a structured and systematic way.
...
The original dataset was published in February 2024, and the revised version was published in October 2025. We intend to update the dataset and correct errors one more time, if we identify errors ourselves or if we have been advised of errors or missing entries. This will be done by the dataset’s first author.

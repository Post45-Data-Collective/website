...
//| echo: false
//| output: false
iww_people_data = fetchData("https://raw.githubusercontent.com/Post45-Data-Collective/website/refs/heads/main/datasets/iowa_writers_workshop/iowa_writers_workshop-people.csv")
```
...
generateTabulatorTableFromCSV(
  "#table-container-iww-people",
  "https://raw.githubusercontent.com/Post45-Data-Collective/website/refs/heads/main/datasets/iowa_writers_workshop/iowa_writers_workshop-people.csv",
  {
    displayedColumns: ["full_name", "given_name", "middle_name", "family_name", "pen_name",
                       "gender", "country", "person_id",
                       "author_authorized_heading", "author_lccn", "author_viaf", "author_wikidata"],
    columnPopups: [
      "Name in 'Lastname, Firstname Middlename' format as listed in University of Iowa records",
      "First name",
      "Middle name (if used)",
      "Last name",
      "Preferred pen name, if different from full name",
      "Provisionally labeled by the PEP team based on name (Male, Female, Gender Nonconforming, Unknown)",
      "Country of origin",
      "Numeric id in the Program Era database",
      "Author's NACO authorized heading",
      "Author's LCCN from id.loc.gov",
      "Author's VIAF cluster number",
      "Author's Wikidata Q number"
    ],
    columnWidths: { "gender": 130, "country": 150 },
    categoryColumns: ["gender", "country"],
    sortColumns: ["full_name"],
    sortOrders: ["asc"],
    buttonContainerId: "#button-container-iww-people",
    rawButtonId: "#download-raw-iww-people",
    urlCopyButtonId: "#copy-url-iww-people",
  }
);
```

<div id="table-container-iww-people" style="height: 600px"></div>

Download Full Data (including hidden columns)

<div>
  <button id="download-raw-iww-people"><i class="fas fa-download"></i> Download Full Data</button>
  <button id="copy-url-iww-people"><i class="fas fa-copy"></i> Copy Full Data URL</button>
</div>

Download Table Data (filtered rows / visible columns)

<div id='button-container-iww-people'>
...
//| echo: false
//| output: false
iww_grads_data = fetchData("https://raw.githubusercontent.com/Post45-Data-Collective/website/refs/heads/main/datasets/iowa_writers_workshop/iowa_writers_workshop-graduations.csv")
```
...
//| output: false
generateTabulatorTableFromCSV(
  "#table-container-iww-grads",
  "https://raw.githubusercontent.com/Post45-Data-Collective/website/refs/heads/main/datasets/iowa_writers_workshop/iowa_writers_workshop-graduations.csv",
  {
    displayedColumns: ["year", "graduate_name", "advisor_name", "genre", "thesis_title",
                       "degree", "program", "institution",
                       "record_id", "graduate_id", "advisor_id",
                       "author_authorized_heading", "author_lccn", "author_viaf", "author_wikidata"],
    columnPopups: [
      "Year of matriculation (range 1920–2014)",
      "Full name of the graduating student",
      "Full name of the student's thesis advisor",
      "Literary genre of the thesis (Poetry, Fiction, Literary Translation, etc.)",
      "Title of the thesis where applicable",
      "Degree awarded (e.g. MFA)",
      "Program (e.g. Writers' Workshop) or major when no specific program applies",
      "Name of degree-granting institution",
      "Numeric id of the graduation record",
      "Foreign key referring to person_id of the graduating student",
      "Foreign key referring to person_id of the thesis advisor",
      "Author's NACO authorized heading",
      "Author's LCCN from id.loc.gov",
      "Author's VIAF cluster number",
      "Author's Wikidata Q number"
    ],
    columnWidths: { "year": 100, "genre": 130, "degree": 110, "program": 180, "institution": 200 },
    rangeSliderColumns: ["year"],
    categoryColumns: ["genre", "program", "institution", "degree"],
    sortColumns: ["year"],
    sortOrders: ["desc"],
    buttonContainerId: "#button-container-iww-grads",
    rawButtonId: "#download-raw-iww-grads",
    urlCopyButtonId: "#copy-url-iww-grads",
  }
);
```

<div id="table-container-iww-grads" style="height: 600px"></div>

Download Full Data (including hidden columns)

<div>
  <button id="download-raw-iww-grads"><i class="fas fa-download"></i> Download Full Data</button>
  <button id="copy-url-iww-grads"><i class="fas fa-copy"></i> Copy Full Data URL</button>
</div>

Download Table Data (filtered rows / visible columns)

<div id='button-container-iww-grads'>
  <button id="download-csv"><i class="fas fa-download"></i> Download CSV</button>
  <button id="download-json"><i class="fas fa-download"></i> Download JSON</button>
  <button id="download-xlsx"><i class="fas fa-download"></i> Download Excel</button>
</div>
...
//| echo: false
//| output: false
iww_hathi_data = fetchData("https://raw.githubusercontent.com/Post45-Data-Collective/website/refs/heads/main/datasets/iowa_writers_workshop/iowa_writers_workshop-hathitrust_metadata.csv")
```
...
//| output: false
generateTabulatorTableFromCSV(
  "#table-container-iww-hathi",
  "https://raw.githubusercontent.com/Post45-Data-Collective/website/refs/heads/main/datasets/iowa_writers_workshop/iowa_writers_workshop-hathitrust_metadata.csv",
  {
    displayedColumns: ["author1", "author1_normalized", "title", "date", "htid", "hathi_rights",
                       "oclc_holdings", "oclc_eholdings", "place", "publisher_marc", "imprint",
                       "isbn", "isbn_most_common_edition", "lccn", "oclc", "oclc_owi", "person_id",
                       "id", "author2", "author3", "author4",
                       "author_authorized_heading", "author_lccn", "author_viaf", "author_wikidata_qid"],
    columnPopups: [
      "Primary author of the work as recorded in HathiTrust",
      "Normalized form of the primary author's name",
      "Short title of the work",
      "Publication date",
      "HathiTrust item (volume) identifier",
      "HathiTrust rights code — see the description below for the full list",
      "OCLC Classify total holdings count",
      "OCLC Classify electronic holdings count",
      "Place of publication (from MARC metadata)",
      "Publisher as recorded in MARC metadata",
      "Full imprint string",
      "ISBN for this volume",
      "ISBN of the most common edition",
      "Library of Congress Control Number for the work",
      "Unique OCLC identifier for this volume in WorldCat",
      "OCLC Classify work identifier",
      "Numeric id of the author in the IWW people dataset",
      "Internal record id",
      "Second author (if applicable)",
      "Third author (if applicable)",
      "Fourth author (if applicable)",
      "Author's NACO authorized heading",
      "Author's LCCN from id.loc.gov",
      "Author's VIAF cluster number",
      "Author's Wikidata Q number"
    ],
    columnWidths: { "date": 110, "hathi_rights": 130 },
    rangeSliderColumns: ["date"],
    numericColumns: ["oclc_holdings", "oclc_eholdings"],
    categoryColumns: ["hathi_rights"],
    sortColumns: ["oclc_holdings"],
    sortOrders: ["desc"],
    buttonContainerId: "#button-container-iww-hathi",
    rawButtonId: "#download-raw-iww-hathi",
    urlCopyButtonId: "#copy-url-iww-hathi",
  }
);
```

<div id="table-container-iww-hathi" style="height: 600px"></div>

Download Full Data (including hidden columns)

<div>
  <button id="download-raw-iww-hathi"><i class="fas fa-download"></i> Download Full Data</button>
  <button id="copy-url-iww-hathi"><i class="fas fa-copy"></i> Copy Full Data URL</button>
</div>

Download Table Data (filtered rows / visible columns)

<div id='button-container-iww-hathi'>
  <button id="download-csv"><i class="fas fa-download"></i> Download CSV</button>
  <button id="download-json"><i class="fas fa-download"></i> Download JSON</button>
  <button id="download-xlsx"><i class="fas fa-download"></i> Download Excel</button>
</div>

...
//| echo: false
//| output: false
major_prizes_winners_data = fetchData("https://raw.githubusercontent.com/Post45-Data-Collective/website/refs/heads/main/datasets/major_literary_prizes/major_literary_prizes-winners_judges.csv")
```
...
generateTabulatorTableFromCSV(
  "#table-container-major-prizes-winners",
  "https://raw.githubusercontent.com/Post45-Data-Collective/website/refs/heads/main/datasets/major_literary_prizes/major_literary_prizes-winners_judges.csv",
  {
    displayedColumns: ["full_name", "gender", "mfa_degree", "role", "prize_year",
                       "prize_name", "prize_amount", "prize_institution", "prize_type",
                       "prize_genre", "elite_institution", "graduate_degree", "stegner",
                       "title_of_winning_book",
                       "author_wikidata", "author_viaf", "author_lccn"],
    columnPopups: [
      "Author's full name (pen names used; most recent name if changed)",
      "Provisionally labeled by the research team based on pronouns used in biographical notes: male, female, nonbinary/he, nonbinary/they, unknown, or No Winner",
      "Name of MFA program institution if individual attended an MFA program; blank or 'No Winner' otherwise",
      "Either 'winner' or 'judge'",
      "Year the prize was awarded",
      "Name of the prize",
      "Prize amount in 2022 dollars (amounts change over time and are not tracked historically)",
      "Nonprofit organization that oversees the prize",
      "'career' (prize awarded for overall career) or 'book' (prize awarded for a specific book)",
      "'poetry,' 'prose,' or 'no genre' (prize has no genre requirement, e.g., MacArthur)",
      "If individual attended one of 12 listed elite institutions (Barnard, Brown, Columbia, Cornell, Dartmouth, Harvard, Princeton, Radcliffe, Stanford, UChicago, UPenn, Yale); blank or 'No Winner' otherwise",
      "'graduate' if individual attended a graduate program (masters, PhD, JD, or medical); blank or 'No Winner' otherwise",
      "'Stegner' if individual received a Wallace Stegner Fellowship at Stanford; blank or 'No Winner' otherwise",
      "Title of the awarded book if prize_type is 'book'; if jury awarded more than one book in the same year, both are listed",
      "Author's Wikidata Q number",
      "Author's VIAF cluster number",
      "Author's LCCN from id.loc.gov"
    ],
    columnWidths: { "gender": "100px", "role": "75px", "mfa_degree": "100px", "prize_name": "100px" },
    currencyColumns: ["prize_amount"],
    rangeSliderColumns: ["prize_year"],
    categoryColumns: ["gender", "role", "prize_name", "prize_institution", "prize_type",
                      "prize_genre", "elite_institution", "mfa_degree", "stegner", "graduate_degree"],
    sortColumns: ["prize_year"],
    sortOrders: ["desc"],
    buttonContainerId: "#button-container-major-prizes-winners",
    rawButtonId: "#download-raw-major-prizes-winners",
    urlCopyButtonId: "#copy-url-major-prizes-winners",
  }
);
```

<div id="table-container-major-prizes-winners" style="height: 600px"></div>

Download Full Data (including hidden columns)

<div>
  <button id="download-raw-major-prizes-winners"><i class="fas fa-download"></i> Download Full Data</button>
  <button id="copy-url-major-prizes-winners"><i class="fas fa-copy"></i> Copy Full Data URL</button>
</div>

Download Table Data (filtered rows / visible columns)

<div id='button-container-major-prizes-winners'>
...
//| echo: false
//| output: false
major_prizes_hathi_data = fetchData("https://raw.githubusercontent.com/Post45-Data-Collective/website/refs/heads/main/datasets/major_literary_prizes/major_literary_prizes-hathitrust_metadata.csv")
```
...
//| output: false
generateTabulatorTableFromCSV(
  "#table-container-major-prizes-hathi",
  "https://raw.githubusercontent.com/Post45-Data-Collective/website/refs/heads/main/datasets/major_literary_prizes/major_literary_prizes-hathitrust_metadata.csv",
  {
    displayedColumns: ["shorttitle", "author", "gender", "inferreddate", "imprintdate",
                       "hathi_id", "hathi_rights", "oclc_holdings", "oclc_eholdings",
                       "author_authorized_heading", "author_lccn", "author_viaf",
                       "author_wikidata_qid", "oclc", "oclc_owi", "person_id"],
    columnPopups: [
      "Short title of the work as listed in HathiTrust",
      "Name of the author of the work as recorded in HathiTrust",
      "Provisionally labeled by the research team based on pronouns used in biographical notes: male, female, nonbinary/he, nonbinary/they, unknown, or No Winner",
      "Earliest publication date for this volume",
      "Date of this specific edition of the text",
      "HathiTrust item (volume) identifier number",
      "HathiTrust rights code — see the description below for the full list of codes",
      "OCLC Classify total holdings count (number of libraries that hold this work)",
      "OCLC Classify electronic holdings count",
      "Author's NACO authorized heading (Name Authority Cooperative)",
      "Author's LCCN from id.loc.gov",
      "Author's VIAF cluster number",
      "Author's Wikidata Q number",
      "OCLC unique identifier for this volume in WorldCat",
      "OCLC Classify work identifier (groups together different editions of the same work)",
      "Unique numeric identifier for the author (matches the winners/judges dataset)"
    ],
    rangeSliderColumns: ["inferreddate", "imprintdate"],
    numericColumns: ["oclc_holdings", "oclc_eholdings"],
    categoryColumns: ["gender", "hathi_rights"],
    sortColumns: ["oclc_holdings"],
    sortOrders: ["desc"],
    buttonContainerId: "#button-container-major-prizes-hathi",
    rawButtonId: "#download-raw-major-prizes-hathi",
    urlCopyButtonId: "#copy-url-major-prizes-hathi",
  }
);
```

<div id="table-container-major-prizes-hathi" style="height: 600px"></div>

Download Full Data (including hidden columns)

<div>
  <button id="download-raw-major-prizes-hathi"><i class="fas fa-download"></i> Download Full Data</button>
  <button id="copy-url-major-prizes-hathi"><i class="fas fa-copy"></i> Copy Full Data URL</button>
</div>

Download Table Data (filtered rows / visible columns)

<div id='button-container-major-prizes-hathi'>
  <button id="download-csv"><i class="fas fa-download"></i> Download CSV</button>
  <button id="download-json"><i class="fas fa-download"></i> Download JSON</button>
  <button id="download-xlsx"><i class="fas fa-download"></i> Download Excel</button>
</div>
